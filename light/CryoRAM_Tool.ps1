...
$DownloadURL = 'https://raw.githubusercontent.com/Cotton059/Light-Help/main/light/CryoRAM.ps1'

# 3. Generate a temporary file path
$rand = Get-Random -Maximum 99999999
$isAdmin = [bool]([Security.Principal.WindowsIdentity]::GetCurrent().Groups -match 'S-1-5-32-544')
$FilePath = if ($isAdmin) { "$env:SystemRoot\Temp\CryoRAM_$rand.ps1" } else { "$env:TEMP\CryoRAM_$rand.ps1" }

Write-Host "[*] Launching CryoRAM Engine..." -ForegroundColor Cyan
