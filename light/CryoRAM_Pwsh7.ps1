...
Fix/Optimization: Cross-Compatible for PowerShell 5.1 & PowerShell 7 (pwsh)
#>

# ==========================================
# UI & Environment Initialization (UTF-8 Enforced)
# ==========================================
# Ensure OS Compatibility (pwsh 7 is cross-platform, this script is Windows-only)
if ($null -ne $IsCoreCLR -and -not $IsWindows) {
    Write-Error "[!] CryoRAM relies on Windows API and is not supported on Linux/macOS."
    Exit
}
...
# Request Administrator Privileges (PWSH 7 Fix: Preserves current host executable)
if (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    Start-Process $CurrentHost -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
    Exit
}
...
# ==========================================
# PWSH 7 Fix: Removed System.Diagnostics dependency from C#. 
# NTSTATUS handles use standard return (PreserveSig = true) to prevent .NET Core crashes.
$WinApiCode = @"
using System;
...
# PWSH 7 Fix: Move Process Name resolution to PowerShell to bypass Add-Type Assembly issues
function Get-ActiveProcessName {
    $pidValue = [WinApi]::GetActiveProcessId()
    if ($pidValue -gt 0) {
...
function Show-Banner {
    ...
    Write-Host " |  Platform : Windows 10 / 11 Optimization (PWSH 7 Ready)  |" -ForegroundColor DarkCyan
    Write-Host " +----------------------------------------------------------+" -ForegroundColor Cyan
    Write-Host ""
}
...
while ($true) {
    ...
    switch ($Choice) {
        ...
        '2' {
            $ActiveApp = Get-ActiveProcessName
